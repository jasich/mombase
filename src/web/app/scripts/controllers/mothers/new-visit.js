...
angular.module('webApp')
  .controller('MothersNewVisitCtrl', function($scope, $injector, $routeParams,
    $location) {
  ...
  .controller('MothersVisitFormCtrl', function($scope, $injector, $routeParams,
    $location) {
    ...
    $scope.isCurrentMoodValid = function() {
      ...
      return mood && mood.name && mood.name.length && mood.mood && mood.mood
        .length;
    }
  ...
  .factory('$accessor', function($parse, $rootScope) {
    function accessor(scope) {
      ...
      scope.set = function(ns, val, conditional) {
        ...
        var getter = $parse(ns);
        if (!conditional || (angular.isFunction(conditional) &&
            conditional.call(scope, getter(scope)) !== false))
